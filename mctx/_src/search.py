...
def simulate(
  ...
  def body_fun(state):
    ...
    #                              state.depth)
    contains_zero = jnp.any(tree.embeddings[1][node_index] == 0, axis=-1)
    action_org = action_selection_fn(action_selection_key, tree, node_index, state.depth)
    action_no_zero = jax.random.randint(action_selection_key, minval=1, maxval=len(tree.children_prior_logits[node_index]), shape=(),dtype = jax.numpy.int32)

    action = jax.lax.cond(contains_zero,
                      lambda _: action_no_zero,
                      lambda _: action_org,
                      operand=None)  # operand 参数在这个例子中未使用，但它是 jax.lax.cond 必需的  
